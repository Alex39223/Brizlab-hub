...
import { ArrowRight, TrendingUp, Users, Wallet, DollarSign, AlertCircle } from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: dbUser.virtualAccounts.length.toString(),
      icon: Wallet,
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="section-title">
          Good day, {dbUser.name?.split(' ')[0] ?? 'there'} 👋
        </h1>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 mt-1">
                Create a virtual bank account to start receiving fiat and converting to USDC
              </p>
            </div>
            <Link href="/onramp" className="btn-primary shrink-0">
        ...
        {dbUser.transactions.length === 0 ? (
          <div className="card text-center py-12">
            ...
            <p className="text-sm text-zinc-600 mt-1">
              Complete KYC and create a virtual account to start.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-surface-3">
                  ...
                  <th className="text-left text-xs text-zinc-500 font-medium px-5 py-3">Destination</th>
                  <th className="text-left text-xs text-zinc-500 font-medium px-5 py-3">Rail</th>
                ...
                {dbUser.transactions.map((tx) => (
                  <tr key={tx.id} className="border-b border-surface-3/50 hover:bg-surface-2/50 transition-colors">
                    <td className="px-5 py-3.5">
                      ...
                      <span className="text-zinc-600 ml-1.5 text-xs">→ {tx.destCurrency.toUpperCase()}</span>
                    </td>
                    <td className="px-5 py-3.5 font-mono-custom text-zinc-400 text-xs">
                      {shortAddress(tx.destAddress)}
                    </td>
                    <td className="px-5 py-3.5 text-zinc-400">
                      {tx.paymentRail?.toUpperCase() ?? '—'}
                    </td>
                    <td className="px-5 py-3.5">
